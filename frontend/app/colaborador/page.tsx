...
const C = {
  ...
  gray: '#6b7280', border: '#e5e7eb', bg: '#f8f7fc',
};
...
function calcTenure(admissao: string, desligamento: string | null) {
  const inicio = new Date(admissao);
  const fim    = desligamento ? new Date(desligamento) : new Date();
...
function tipoConfig(tipo: string) {
  ...
  if (t.includes('reajuste') || t.includes('salário') || t.includes('salario'))
...
function IconSearch({ c }: { c: string }) {
  return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>;
}
...
export default function ColaboradorPage() {
  ...
  function limpar() {
    setColab(null); setHistorico([]); setQuery('');
  ...
  /* ── Timeline: admissão → histórico → desligamento ── */
    ...
    <div className="min-h-screen" style={{ backgroundColor: C.bg }}>
      ...
      <main className="max-w-screen-xl mx-auto px-6 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-sm mt-1" style={{ color: C.gray }}>Busque pelo nome para ver o perfil completo e a linha do tempo</p>
        </div>
        ...
        <div id="search-wrap" className="relative mb-8 max-w-xl">
          <div className="flex items-center gap-3 bg-white rounded-2xl px-4 py-3 shadow-md border"
            ...
            <span style={{ color: C.gray }}><IconSearch c={C.gray} /></span>
            <input
              ref={inputRef}
              type="text"
              value={query}
                ...
                <button key={s.id_colaborador} onClick={() => selecionar(s)}
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white flex-shrink-0"
                      style={{ background: `linear-gradient(135deg, ${C.purple}, ${C.teal})` }}>
                      {iniciais(s.nome)}
                    </div>
        ...
        {colab && !loadingProf && (
          <div className="flex flex-col gap-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border overflow-hidden" style={{ borderColor: C.border }}>
              {/* Faixa roxa de topo */}
              <div className="h-24 relative" style={{ background: `linear-gradient(135deg, ${C.dark} 0%, ${C.purple} 50%, ${C.teal} 100%)` }}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[340px_1fr] gap-6 items-start">
              ...
              <div className="bg-white rounded-2xl shadow-sm border p-6" style={{ borderColor: C.border }}>
                ...
                {eventosTimeline.length === 0 ? (
                  <p className="text-sm text-center py-8" style={{ color: C.gray }}>
                    Nenhum evento registrado no histórico.
                  </p>
                ) : (
                  <div className="relative">
                    {/* Linha vertical */}
                    <div className="absolute left-5 top-0 bottom-0 w-0.5" style={{ background: `linear-gradient(to bottom, ${C.purple}, ${C.teal})` }} />
                          ...
                          <div key={ev.id} className="relative flex gap-5 pb-6">
                            ...
                            <div className="relative z-10 flex-shrink-0">
                              <div className="w-10 h-10 rounded-full border-2 border-white shadow flex items-center justify-center"
                                style={{ background: cfg.bg, outlineColor: cfg.cor }}>
                                <Icone c={cfg.cor} />
                              </div>
                              {isFirst && (
                            ...
                            {/* Card do evento */}
                            <div className="flex-1 rounded-xl border p-4 mb-0" style={{ background: cfg.bg, borderColor: `${cfg.cor}30` }}>
                              <div className="flex items-start justify-between flex-wrap gap-2 mb-2">
                                ...
                                <span className="text-xs font-semibold" style={{ color: C.gray }}>
                                  ...
                                  {ev.data_fim ? ` → ${fmtData(ev.data_fim)}` : ''}
                                </span>
                              </div>
                              ...
                              <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2">
                                {ev.departamento && (
                                  <span className="text-xs" style={{ color: C.gray }}>
                                    <span className="font-semibold">Dept:</span> {ev.departamento}
                                  </span>
                                )}
                                {ev.area && ev.area !== ev.departamento && (
                                  <span className="text-xs" style={{ color: C.gray }}>
                                    <span className="font-semibold">Área:</span> {ev.area}
                                  </span>
                                )}
                                {ev.unidade && (
                                  <span className="text-xs" style={{ color: C.gray }}>
                                    <span className="font-semibold">Unidade:</span> {ev.unidade}
                                  </span>
                                )}
                                {ev.vinculo && (
                                  <span className="text-xs" style={{ color: C.gray }}>
                                    <span className="font-semibold">Vínculo:</span> {ev.vinculo}
                                  </span>
                                )}
                                {ev.centro_custo && (
                                  <span className="text-xs" style={{ color: C.gray }}>
                                    <span className="font-semibold">CC:</span> {ev.centro_custo}
                                  </span>
                                )}
                              </div>

                              {ev.motivo && ev.motivo !== 'Entrada na empresa' && (
                                <p className="text-xs mt-2 italic" style={{ color: C.gray }}>
                                  Motivo: {ev.motivo}
                                </p>
                              )}
